...
<script>
	$(function() {
		...
		$("input:eq(0)").on('keyup',
				function() {
					$("#massage").empty();//처음에 pattern에 적합하지 않은 경우 메시지 출력 후 적합한 데이터를 입력해도
											//계속 같은 데이터 출력하므로 이벤트 시작할 때마다 비워둔다.
					//[A-Za-z0-9_]의 의미가 \w
					var pattern = /^\w{5,12}$/;
		...
		$("input:eq(6)").on('keyup',
				function() {
					$("#email_message").empty();
					//[A-Za-z0-9_]의 동일한 것이 \w 입니다.
					//+는 1회 이상 반복을 의미하고 {1,}와 동일합니다.
					//\w+ sms [A-Za-z0-9]를 1개이상 사용하라는 의미입니다.
					var pattern = /^\w+@\w+[.]\w{3}$/;
