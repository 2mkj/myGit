...
select * 
from board join myboard 
where board.board_num = myboard.board_name;

insert into myboard select board_num, board_name from board

SELECT * FROM BOARD
delete from BOARD where board_num = 21;
SELECT nvl(max(board_num),0)+1 from board
select * from board where BOARD_NAME ='admin1';
...
from (select rownum rnum, j.*
	  from (
			...
			from board left outer join (select comment_board_num, count(*) cnt
				 						...
				 						group by comment_board_num)
			on board_num = comment_board_num
			where board_name = 'admin1'and board_subject like '1' or board_content like '1'
			order by BOARD_RE_REF desc,   
			BOARD_RE_SEQ asc          
	 		) j
	  where rownum <= 10 
...
select count(*) from board where board_name='admin1' and board_subject like '1' or board_content like '1'