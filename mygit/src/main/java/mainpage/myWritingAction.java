...
public class myWritingAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("search_word") == null
			...
			// 총 리스트 수를 받아옵니다.
			listcount = boarddao.getmyListCount(id);
			boardlist = boarddao.getmyWriting(id, page, limit);
		} else { // 검색을 클릭한 경우
			search_word = request.getParameter("search_word");
			listcount = boarddao.getmyListCount(id, search_word);
			boardlist = boarddao.getmyWriting(id, search_word , page, limit);
		}
		...
		if(state == null) {
			...
			forward.setPath("mypage/myWriting.jsp");
			return forward; 
