...
public class ManagerListAction implements ManagerAction{
	...
	public ManagerForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 리스트를 받아옵니다.
		managerlist = dao.getManagerList(page,limit);
		
		/*
		 * 총 페이지 수
	=(DB에 저장된 총 리스트의 수 + 한 페이지에서 보여주는 리스트의 수 -1)/한 페이지에서 보여주는 리스트 수
	...
	*/
		 ...
		 * 예로 마지막 페이지 그룹 [21]~[30]인 경우
		 * 시작 페이지는 21(startpage=21)와 마지막 페이지는 30(endpage=30)이지만
		 * 최대 페이지(maxpage)가 25이면 [21]~[25]까기만 표시되도록 합니다.
		 */
		if(endpage > maxpage)
			endpage = maxpage;
		...
		if(state == null) {
			...
			object.addProperty("page", page); // {"page" : 변수 page값} 형식으로 저장
			...
			// List => JsonElement
			JsonElement je = new Gson().toJsonTree(managerlist);
			System.out.println("Managerlist="+je.toString());
			object.add("Managerlist", je);
			
			response.setContentType("text/html;charset=utf-8");
			response.getWriter().append(object.toString());
			System.out.println(object.toString());
			return null; // response에 값을 담아서 보냈기때문에 return 값으로 null이된다.
		}
	}
