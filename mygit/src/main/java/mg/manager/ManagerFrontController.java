...
public class ManagerFrontController extends javax.servlet.http.HttpServlet{
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 	  http://localhost:8088/JspProject/login.net로 요청하면 RequestURL는
		...
		//초기화
		ManagerForward forward = null;
		ManagerAction action = null;
		
		switch(command) {
		
		case "/managerList.mg":
			action = new ManagerSearchAction();
			break;
		case "/managerInfo.mg":
			action = new ManagerInfoAction();
			break;
		case "/managerDelete.mg":
